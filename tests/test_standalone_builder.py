"""Tests for the standalone dual-company 3-statement builder.

Self-contained: constructs synthetic ``NormalizedFacts`` / ``StatementSet`` and
does NOT depend on ``src/edgar`` (the both-companies smoke test try-imports it
and skips gracefully if that sibling is not yet built).

The builder is company-agnostic — it drives a projection purely off
``NormalizedFacts`` + ``ProjectionAssumptions`` — so the SAME code models both
the acquirer and the target; the both-companies smoke test exercises that.
"""
...
from src.standalone import ThreeStatementBuilder, balance_check
...
# --------------------------------------------------------------------------- #
# Golden tiny model — every asserted number hand-computed in the docstring.
# --------------------------------------------------------------------------- #
def _one_year_hist() -> StatementSet:
    """A single balanced historical year, built by hand.

    Assets  = cash 200 + AR 100 + inventory 150 + PP&E 550        = 1000
    Liab    = AP 80 + short-term debt 20 + long-term debt 300     =  400
    Equity  = retained earnings 400 + common stock 200            =  600
    => Assets 1000 = Liabilities 400 + Equity 600  (balanced)
    """
...
def _golden_assumptions(n: int = 5) -> ProjectionAssumptions:
    """Zero interest rates so net income is deterministic (no circularity to
    hand-solve): the revolver plug then never touches the income statement, and
    Year-1 revenue/EBIT/NI can be asserted to the cent."""
...
def test_golden_year1_income_statement_to_the_cent():
    """Year-1 hand math (interest rates = 0):

    revenue = 1000 * 1.10                        = 1100.00
    COGS    = 1100 * (1 - 0.40)                  =  660.00
    gross   = 1100 - 660                         =  440.00
    SG&A    = 0.10 * 1100                         =  110.00
    R&D     = 0.05 * 1100                         =   55.00
    EBIT    = 440 - 110 - 55                      =  275.00
    pretax  = EBIT (no interest)                  =  275.00
    tax     = 0.25 * 275                          =   68.75
    NI      = 275 - 68.75                         =  206.25
    """
    hist = _one_year_hist()
    stmts = ThreeStatementBuilder().project(hist, _golden_assumptions())
    assert stmts.n_hist == 1
    assert len(stmts.periods) == 6
    assert [p.fy for p in stmts.periods[1:]] == [2024, 2025, 2026, 2027, 2028]

    assert stmts.series(LineItem.REVENUE)[1] == pytest.approx(1100.0)
    assert stmts.series(LineItem.COST_OF_REVENUE)[1] == pytest.approx(660.0)
    assert stmts.series(LineItem.GROSS_PROFIT)[1] == pytest.approx(440.0)
    assert stmts.series(LineItem.SGA)[1] == pytest.approx(110.0)
    assert stmts.series(LineItem.RND)[1] == pytest.approx(55.0)
    assert stmts.series(LineItem.OPERATING_INCOME)[1] == pytest.approx(275.0)
    assert stmts.series(LineItem.PRETAX_INCOME)[1] == pytest.approx(275.0)
    assert stmts.series(LineItem.INCOME_TAX_EXPENSE)[1] == pytest.approx(68.75)
    assert stmts.series(LineItem.NET_INCOME)[1] == pytest.approx(206.25)


def test_golden_revenue_compounds():
    hist = _one_year_hist()
    stmts = ThreeStatementBuilder().project(hist, _golden_assumptions())
    rev = stmts.series(LineItem.REVENUE)
    assert rev[1] == pytest.approx(1000.0 * 1.1)
    assert rev[5] == pytest.approx(1000.0 * 1.1**5)
...
def test_golden_retained_earnings_roll():
    ...
    for t in range(1, 6):
        ...
        assert re[t] == pytest.approx(expected)
        prev = re[t]
...
def test_golden_ppe_roll():
    ...
    for t in range(1, 6):
        ...
        assert ppe[t] == pytest.approx(expected)
        prev = ppe[t]
...
def test_cfs_ending_cash_ties_to_bs_cash():
    ...
    prev = 200.0
    for t in range(1, 6):
        assert cash[t] == pytest.approx(prev + net_change[t])
        prev = cash[t]


def test_interest_circularity_converges_and_balances():
    """With non-zero rates the interest<->debt/cash loop must still resolve and
    the balance sheet must stay square every period."""
    hist = _one_year_hist()
    a = _golden_assumptions()
    a.interest_rate_on_debt = 0.05
    a.interest_rate_on_cash = 0.02
    stmts = ThreeStatementBuilder().project(hist, a)
    # interest expense is stored positive and non-zero given outstanding debt
    for t in range(1, 6):
        assert stmts.series(LineItem.INTEREST_EXPENSE)[t] > 0
    for resid in balance_check(stmts)[1:]:
...
def test_project_requires_history():
    ...
    with pytest.raises(ValueError):
        ThreeStatementBuilder().project(empty, _golden_assumptions())


# --------------------------------------------------------------------------- #
# PROPERTY TEST — seeded random (no hypothesis dep). The headline invariant.
# --------------------------------------------------------------------------- #
def _random_hist(rng: random.Random) -> StatementSet:
    rev = rng.uniform(500, 5000)
    rows: dict[LineItem, list[float | None]] = {li: [None] for li in LineItem}
    rows[LineItem.REVENUE] = [rev]
    rows[LineItem.CASH] = [rng.uniform(50, 800)]
    rows[LineItem.ACCOUNTS_RECEIVABLE] = [rng.uniform(20, 500)]
    rows[LineItem.INVENTORY] = [rng.uniform(20, 500)]
    rows[LineItem.PPE_NET] = [rng.uniform(100, 3000)]
    rows[LineItem.OTHER_NONCURRENT_ASSETS] = [rng.uniform(0, 500)]
    rows[LineItem.ACCOUNTS_PAYABLE] = [rng.uniform(20, 400)]
    rows[LineItem.SHORT_TERM_DEBT] = [rng.uniform(0, 300)]
    rows[LineItem.LONG_TERM_DEBT] = [rng.uniform(0, 2000)]
...
def test_property_revolver_keeps_cash_above_min():
...
# --------------------------------------------------------------------------- #
# BOTH-COMPANIES smoke test — models acquirer + target with the SAME code.
# Try-imports src/edgar (built in parallel); skips if not yet importable.
# --------------------------------------------------------------------------- #
def test_both_companies_smoke_via_edgar():
    edgar = pytest.importorskip("src.edgar", reason="src/edgar not built yet (parallel work)")
    load = (
        getattr(edgar, "load_normalized_facts", None)
        or getattr(edgar, "load_facts", None)
        or getattr(edgar, "load_company_facts", None)
    )
    if load is None:
        pytest.skip("src.edgar has no facts loader yet")
    ...
    for ticker in ("SNPS", "ANSS"):
        try:
            facts = load(ticker)
        except Exception as exc:  # loader present but no cached fixtures yet
            pytest.skip(f"no cached facts for {ticker}: {exc}")
        hist = builder.build_historical(facts)
        assert hist.n_hist >= 1
        stmts = builder.project(hist, a)
        for resid in balance_check(stmts)[hist.n_hist :]:
