...
precedent CSV parsing, and the fairness ``implied_range_from_multiple`` helper.

Ported from the thesis project's test_comps_engine.py. All fixtures are
synthetic NormalizedFacts built in-test (no dependency on the EDGAR layer). The
curated-precedents-file tests are replaced by a temp-CSV test (proforma has no
committed footwear precedents file); every other case ports verbatim.
"""
...
import pytest
from src.comps import CompsEngine, implied_range_from_multiple
...
# ---------------------------------------------------------------------------
# implied_range_from_multiple (proforma addition): the "Selected Companies" /
# "Selected Transactions" reproduction — a disclosed multiple RANGE applied to a
# subject metric -> implied per-share (low, high).
# ---------------------------------------------------------------------------
def test_implied_range_from_ev_multiple_hand_computed():
    """EV/EBITDA range 18.0x-24.0x on EBITDA=500, net_debt=200, shares=100.
    low  : EV = 500*18 = 9000 ; equity = 9000-200 = 8800 ; /100 = 88.00
    high : EV = 500*24 = 12000 ; equity = 12000-200 = 11800 ; /100 = 118.00
    """
    subject = _facts("SUB")
    low, high = implied_range_from_multiple(
        subject, 500.0, 18.0, 24.0, net_debt=200.0, shares=100.0
    )
    assert (low, high) == (88.0, 118.0)


def test_implied_range_from_equity_multiple_hand_computed():
    """Equity multiple (e.g. price/LFCF) 20.0x-25.0x on LFCF=300, shares=100
    (no net-debt bridge):
        low  : equity = 300*20 = 6000 ; /100 = 60.00
        high : equity = 300*25 = 7500 ; /100 = 75.00
    net_debt is passed but must be ignored for the equity-multiple path.
    """
    subject = _facts("SUB")
    low, high = implied_range_from_multiple(
        subject, 300.0, 20.0, 25.0, net_debt=999.0, shares=100.0, is_equity_multiple=True
    )
    assert (low, high) == (60.0, 75.0)


def test_implied_range_orders_ascending_regardless_of_input_order():
    subject = _facts("SUB")
    # Pass high multiple first; result must still be (low, high).
    low, high = implied_range_from_multiple(
        subject, 500.0, 24.0, 18.0, net_debt=200.0, shares=100.0
    )
    assert (low, high) == (88.0, 118.0)


def test_implied_range_requires_positive_shares():
    subject = _facts("SUB")
    with pytest.raises(ValueError, match="positive share count"):
        implied_range_from_multiple(subject, 500.0, 18.0, 24.0, net_debt=0.0, shares=0.0)


# ---------------------------------------------------------------------------
# Precedent CSV parsing on a temp file: every row carries a source; blanks None.
# ---------------------------------------------------------------------------
def test_load_precedents_parses_and_sources(tmp_path):
    ...
    assert rows[0].acquirer == "A"
    assert rows[0].target == "B"
    assert rows[0].ev == 1000.0
    assert rows[0].ev_revenue == 2.5
    assert rows[0].ev_ebitda is None  # blank -> honest unknown
    assert rows[0].source == "src-1"
