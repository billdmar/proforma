...
Ported near-verbatim from the thesis project's ``src/edgar/normalize.py`` (cited
in docs/DESIGN.md); the priority-ordered alias map already carries the SNPS/ANSS
tags (both file ``Revenues``; SNPS also files
``RevenueFromContractWithCustomerExcludingAssessedTax``), so the selection
algorithm reused here loads both deal companies unchanged.
...
# ---------------------------------------------------------------------------
# Priority-ordered tag alias map (reused from thesis; covers the SNPS/ANSS
# fixtures — both large-cap software filers using the standard us-gaap/dei tags).
