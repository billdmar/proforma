...
Ported near-verbatim from the thesis project's ``src/edgar/client.py`` (cited in
docs/DESIGN.md), retargeted to proforma's fixtures and User-Agent. Fair-access is
law here (see config.settings):
* every request carries the ``proforma-research`` User-Agent,
...
class EdgarClient:
    ...
    def lookup_cik(self, ticker: str) -> str:
        ...
        Resolves from the cached SEC ticker index first. The deal target (ANSS)
        is not present in the committed ``company_tickers.json`` (it dropped from
        the SEC ticker feed around the merger), so we fall back to the frozen
        per-deal CIK constants in ``config.settings`` — a sourced constant, not a
        guessed value. Unknown tickers still raise ``KeyError`` (honest-unknown:
        we do not fabricate a CIK).
        """
        index = self._load_ticker_index()
        key = ticker.upper()
        if key in index:
            return index[key]["cik"]
        fallback = self._deal_cik_fallback(key)
        if fallback is not None:
            return fallback
        raise KeyError(f"ticker not found in company_tickers.json: {ticker!r}")

    @staticmethod
    def _deal_cik_fallback(ticker: str) -> str | None:
        """CIK for a deal party from the frozen settings constants, else None."""
        deal_ciks = {
            settings.ACQUIRER_TICKER.upper(): settings.ACQUIRER_CIK,
            settings.TARGET_TICKER.upper(): settings.TARGET_CIK,
        }
        return deal_ciks.get(ticker)
