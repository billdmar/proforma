"""Dual-company standalone builder: historical assembly + linked 5-year projection.

Company-agnostic 3-statement model. It operates purely on ``NormalizedFacts`` +
``ProjectionAssumptions``, so the SAME code models both the acquirer (SNPS) and
the target (ANSS) — nothing here knows which company it is looking at.

Ported from the thesis project's ``src/statements/builder.py`` (cited in
docs/DESIGN.md); the mechanics are preserved because the fairness differential
must run the advisors' disclosed assumptions through the identical engine.
