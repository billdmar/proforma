...
Ported from the thesis project (``src/statements/helpers.py``; cited in
docs/DESIGN.md). These four helpers were duplicated character-for-character
across ``src.valuation`` and ``src.lbo``; consolidating them keeps one
definition of the net-debt / share-count / D&A-add-back conventions (and their
honest-unknown handling) so the two engines can't drift apart. The comps engine
reads from ``NormalizedFacts`` (a different source shape), so it keeps its own
accessor.
"""
