"""Banker-convention cell styling helpers for the merger-model workbook writer.

Ported near-verbatim from the thesis project's ``src/workbook/styles.py`` (cited
in docs/DESIGN.md) and kept identical where the house rules are identical.
Encapsulates the conventions from docs/WORKBOOK_SPEC.md so the writer stays
declarative:

* **Blue font** = a hard input (only on the Assumptions tab + Cover metadata).
* **Black font** = a live formula (everywhere else) or a labeled engine-output /
  historical value.
* Number formats: currency ``#,##0;(#,##0)``, percent ``0.0%``, per-share
  ``0.00``, multiple ``0.0x``, shares ``#,##0``, year ``0``.

The three cell-writing helpers are the enforcement point: ``input_cell`` writes a
blue number, ``formula_cell`` writes a black formula string (**must start with
``=``** — the anti-hardcode enforcement point), ``value_cell`` writes a black
reported/engine-output number. Using them consistently is what makes the
"formulas-not-values" audit and the cell-level differential pass.
"""
...
def formula_cell(ws, coord: str, formula: str, fmt: str = FMT_CURRENCY) -> Cell:
    ...
    Raising on a non-formula string is the anti-hardcode enforcement point: a
    computed cell can never silently carry a baked number.
    """
...
def value_cell(ws, coord: str, value, fmt: str = FMT_CURRENCY) -> Cell:
    """Write a reported/engine-output VALUE (the XBRL tie-out & anchor surface).

    Black font (it is not a blue input), plain number. Used for historical
    statement lines sourced from SEC facts and for labeled engine-output anchors
    (e.g. breakeven synergies) that are solve results, not formula chains.
    """
    cell = ws[coord]
    cell.value = value
    cell.font = Font(color=BLACK, name="Calibri", size=11)
