...
Ported from the thesis project (``src/comps/engine.py``; cited in
docs/DESIGN.md), with one proforma-specific addition for the fairness
"Selected Companies" / "Selected Transactions" reproduction
(``implied_range_from_multiple``, below).

Concrete implementation of the ``CompsEngine`` protocol (src/interfaces.py).

Design notes (the judgment that matters in an interview):
...
def implied_range_from_multiple(
    ...
    *,
    ...
    """Apply a disclosed multiple RANGE to a subject metric → implied per-share range.

    This is the fairness "Selected Companies" / "Selected Transactions"
    reproduction: an advisor discloses a chosen multiple range (e.g. EV/EBITDA
    18.0×–24.0×, or a price/LFCF range) applied to a subject metric, and we
    reproduce the implied per-share (low, high) range they report.

    * **EV multiple** (``is_equity_multiple`` False, the default):
      ``implied_EV = metric_value × mult``; ``implied_equity = implied_EV −
      net_debt``; ``per_share = implied_equity / shares``.
    * **Equity multiple** (``is_equity_multiple`` True; e.g. price / LFCF):
      the multiple applies directly to equity — ``implied_equity =
      metric_value × mult``; ``per_share = implied_equity / shares`` (no net-debt
      bridge).

    ``net_debt`` is supplied by the caller (positive = net debt) so the fairness
    harness can pass the disclosed balance-sheet figure; ``_total_debt`` /
    ``_cash_and_st`` remain available to derive it from ``subject`` when needed.
    ``shares`` must be > 0. The result is ordered (low, high) regardless of the
    order the multiples are passed in.

    Args:
        subject: the subject company's facts (carried for symmetry / callers that
            derive metrics from it; the metric itself is passed explicitly).
        metric_value: the subject metric the multiple is applied to (e.g. EBITDA,
            or LFCF for an equity multiple).
        low_mult: low end of the disclosed multiple range.
        high_mult: high end of the disclosed multiple range.
        net_debt: net debt for the EV→equity bridge (ignored for equity multiples).
        shares: diluted share count for the per-share bridge (must be > 0).
        is_equity_multiple: True when the multiple is an equity multiple.

    Returns:
        (low_price, high_price) implied per-share range, ascending.
    """
    if shares <= 0:
        raise ValueError(
            "implied_range_from_multiple requires a positive share count for the per-share bridge."
        )

    def per_share(mult: float) -> float:
        implied_value = metric_value * mult
        implied_equity = implied_value if is_equity_multiple else implied_value - net_debt
        return implied_equity / shares

    a = per_share(low_mult)
    b = per_share(high_mult)
