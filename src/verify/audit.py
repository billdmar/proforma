...
Opens the merger-model workbook with openpyxl (``data_only=False`` so formulas
are visible as formulas) and enforces the house rules from docs/WORKBOOK_SPEC.md:
...
* **No baked values in formula regions.** Every cell the differential maps
  ...
  defect. This sub-check needs the bundle to build the map; skipped when no
  model is supplied.
* **Blue inputs are confined.** A blue-font cell (input) may appear ONLY on the
  Assumptions tab and the Cover metadata cells (the announced / expected-close
  cells). Everywhere else a blue cell is a banker-convention breach.
...
Ported from the thesis project's ``src/verify/audit.py`` (cited in
docs/DESIGN.md), adapted to the :class:`~src.interfaces.MergerModelBundle` and
this writer's sheet constants + named ranges.
"""
...
# The Cover metadata cells allowed to carry input styling off the Assumptions
# tab (the announced / expected-close cells), per WORKBOOK_SPEC §Banker
# conventions. Proforma's writer keeps these plain-font today, so this is a
# defensive allowance: it documents which non-Assumptions cells are exempt.
_ALLOWED_COVER_INPUTS = {"B3", "B4"}
...
def audit_workbook(workbook_path: str, model: MergerModelBundle | None = None) -> AuditReport:
    ...
    # (b) Blue inputs only on Assumptions + the allowed Cover metadata cells.
