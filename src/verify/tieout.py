...
Ported near-verbatim from the thesis project's ``src/verify/tieout.py`` (cited
in docs/DESIGN.md); the historical ``StatementSet`` it reconciles is produced by
``src.standalone.ThreeStatementBuilder`` here (the thesis equivalent was
``src.statements.builder``). Both build against the same frozen ``src.schema``.
"""
...
def balance_sheet_ties(facts: NormalizedFacts, *, tol: float = 1.0) -> list[tuple[int, float]]:
    ...
    filer did not tag TotalLiabilities, it is inferred as TotalAssets -
    TotalEquity, so this degenerates to an equity/asset presence check for those
    filers rather than a false failure.
    """
